...
then
	...
	echo "--------------------------------------"
	local dmesg_result=$(dmesg|grep LSI|grep scsi)
	echo "$dmesg_result"
	echo "--------------------------------------"	
	set -- $dmesgresult

	# Extract the line with 'scsi host'
	host_number=$(echo "$dmesg_result" | sed -n 's/.*scsi host\([0-9]\+\).*/\1/p')
	
	echo "It appears SCSI card is located in Host $host_number"
	read -p "Do you want to try Force SCSI Host Bus Scan? [Y/n]" yn
	case $yn in
		[Yy]* ) echo "- - -" > /sys/class/scsi_host/host$host_number/scan;;
		[Nn]* ) exit 1;;
	esac
